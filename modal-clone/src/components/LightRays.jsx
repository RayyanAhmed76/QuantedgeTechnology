...
export default function LightRays({
  ...
  useEffect(() => {
    ...
    // Hero is above the fold — start immediately, IO is a backup for off-screen cases
    setIsVisible(true)
  ...
  useEffect(() => {
    ...
    const initializeWebGL = async () => {
      ...
      try {
        ...
        // LightRays shaders are WebGL1 (gl_FragColor). Force webgl: 1 —
        // OGL defaults to WebGL2, which compiles these shaders to a black/no-op canvas.
        const renderer = new Renderer({
          dpr: Math.min(window.devicePixelRatio || 1, reduceMotion ? 1 : 1.5),
          alpha: true,
        ...
        gl.enable(gl.BLEND)
        // Additive: avoid SRC_ALPHA (double-dims rgb when alpha mirrors strength)
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  // Keep more mid-screen energy so lime rays stay readable on phones
  fragColor.x *= 0.25 + brightness * 0.85;
  fragColor.y *= 0.55 + brightness * 0.7;
  fragColor.z *= 0.35 + brightness * 0.55;
  ...
  fragColor.rgb *= raysColor * max(intensity, 0.0);
...
void main() {
  ...
  gl_FragColor  = color;
        ...
        console.warn('LightRays init failed:', error)
        onErrorRef.current?.()
      }
    }
